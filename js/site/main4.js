...
// fancyr/fancyrd button and add-to-list popup rating popup
jQuery(function($){
    var dlg_list = $.dialog('add-rating'), $list_popup = dlg_list.$obj, loading_txt, $usertext, $coltext, $tpl_user;
    ...
    dlg_list.$obj
        .on({
            ...
            close : function(){
                ...
                $list_popup
                    ...
                    //.find('>.add_to_list').hide().end();
                    
                    $list_popup.find('.item-categories .popup_product_id').val(0);
            },
        ...
        .on('click', '.btn-unfancy', function(event){
            ...
            $.ajax({
                ...
                success  : function(response){
                    if(response.status_code==1){
                        $('.button.fancyrd[tid='+tid+']').removeAttr('rtid').removeClass('fancyrd').addClass('fancy').html('<span><i></i></span>'+gettext(likeTXT));
                        dlg_list.close();
                    }
    ...
    // when click "Fancyr it" or "Fancyr'd" button
    $('#content,#sidebar,#slideshow-box, .profile, .shoppage')
        .delegate('.button.fancyrd, .button.fancyr, .button2.fancylist', 'click', function(event){

            var $this = $(this),
                tid  = $this.attr('tid') || null,
                rtid = $this.attr('rtid') || null,
                sl   = $this.attr('show_add_to_list') || null,
                rt   = $this.attr('rating') || null,
                login_require = $this.attr('require_login'),
...
*/            
            ...
            // ajax call for rating popup
            if(rt){
                //$list_popup.find('.list-categories').html('Loading...');
                $list_popup.find('#rating-stars-cont').hide();
                $list_popup.find('.list-box').addClass("list-box-loader");
                $list_popup.find('.list-box').prepend(global_ajax_loader);
                $list_popup.find('.item-categories .popup_product_id').val(tid);
                $.ajax({
                    type:'POST',
                    url:baseURL+'site/user/get_prduct_rating',
                    data:{tid:tid, uid:$("#logged_in_user_id").val()},
                    dataType:'json',
                    success:function(response){
                        if(response.status_code == 1){
                            $list_popup.find('#example-fontawesome-o').attr("data-current-rating", response.product_rating);
                            if(response.product_rating>0){
                                $('#example-fontawesome-o').barrating('set', response.product_rating);
                                $list_popup.find('.ajax-loader-dy').remove();
                                $list_popup.find('#rating-stars-cont').show("slow");
                            }else{
                                $('#example-fontawesome-o').barrating('clear');
                                $list_popup.find('.active-rating').html("0");
                                $list_popup.find('.ajax-loader-dy').remove();
                                $list_popup.find('#rating-stars-cont').show("slow");
                            }
                        }
            ...
            $.ajax({
                ...
                success:function(response){
                    if(response.status_code == '1'){
                        ...
                        $this.toggleClass('fancyr fancyrd');
                    }
                }
            });
            $this.removeClass('loading');
        })
        .delegate('.button.fancyrd,.button.fancyr', 'mouseenter mouseleave', function(event){
            var $this = $(this);
...
jQuery(function($){
    ...
    $('#content,#sidebar,#slideshow-box, .profile, .shoppage')
        .delegate('.button.fancyd, .button.fancy, .button2.fancylist', 'click', function(event){

            var $this = $(this),
                tid  = $this.attr('tid') || null,
                rtid = $this.attr('rtid') || null,
                sl   = $this.attr('show_add_to_list') || null,
                rt   = false,//$this.attr('rating') || null,
                login_require = $this.attr('require_login'),
                checkbox_url  = '/_get_list_checkbox.html?t='+(new Date).getTime(),
                available_dlg = !!dlg_list.$obj.length;
...
*/            
            ...
            else if((dlg_list.$obj.attr('show_when_fancy') == 'true') || sl) {
                $this.removeAttr('show_add_to_list');
                //$list_popup.find('#rating-container').hide();
                //$list_popup.find('.default').css("display", "none");
                //$list_popup.find('.add_to_list').show();
